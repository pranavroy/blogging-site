...
if($_SESSION["authuser"]!=1){//for checking the session user as authorized or not
  header("Location:user_login.php");//if not then redirect it to login again
}
...
if(isset($_POST['submit']))//when submit button is clicked
{
  ...
  if(!empty($title) && !empty($content)){
    ...
    $name=$_SESSION["uname"];
    $sql = "INSERT INTO blog (title, content, time, name,status) VALUES ('$title','$content','$time','$name','0')";
    $conn->exec($sql);
    header("Location:newuser_index.php");
  }
...
{   
    session_unset();
    header("Location:newhome_page.php");//redirect it to home page
...
<input  type="submit" name="logout" class="btn btn-primary" value="Logout">
    ...
    <div class="container">
      ...
      <div class="form-group"><b>
        Title<b> <input type="text" class="form-control" name="title" required></div>
          <br>
          <div class="form-group">
            Blog <textarea class="form-control" name="content" rows="20" cols="30" required></textarea></div>
            <br><br>
            <input type="submit" class="btn btn-warning" name="submit" value="Post"></form>
