...
if($_SESSION["authuser"]!=1){//for checking the session user as authorized or not
  header("Location:user_login.php");//if not then redirected to login page again
}
include("db.php");//including database connection file
$id=$_GET['id'];
if(isset($_POST['submit']))//when update button is clicked
{
  ...
  if(!empty($title) && !empty($content)){
    ...
    $name=$_SESSION["uname"];
    $sql="update blog set title=:title,content=:content,time=:time,name=:name,status='0' where id=:id";
...
{   
    session_unset();
    header("Location:newhome_page.php");//redirect to home page
...
<input  type="submit" name="logout" class="btn btn-primary" value="Logout">
    ...
    <form action="" method="post">
      <div class="form-group"><b>
        Edit Title <b><input type="text" value="<?php  echo ($row['title']); ?>" class="form-control" name="title" required></div>
          <br>
          <div class="form-group">
            Edit Content <textarea class="form-control" name="content" rows="20" cols="50" required><?php  echo ($row['content']); ?>
          </textarea></div>
          <br><br>
