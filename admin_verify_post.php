...
if($_SESSION["authuser"]!=2){//for checking the session user as authorized or not
    header("Location:admin_login.php");//if not then referred again to login
}
...
<body>
    ...
    <div class="container">
          ...
          <div class="col-sm-2 paddingTop20">
            <nav class="nav-sidebar">
                <ul class="nav">
                    ...
                    <li class="active"><a href="admin_verify_post.php">Verify Post</a></li>
        ...
        <?php
        try {
           $stmt = $conn->prepare("SELECT * FROM blog where status=0"); 
           $stmt->execute();
    ...
    <div class="col-md-10 blogShort" style="background-color:#f6f1ec">
        <?php while($row = $stmt->fetch(PDO::FETCH_ASSOC)){ ?>
           ...
           <?php echo "<a href='admin_delete_post.php?id=".$row['id']."'>Delete</a>";?><br>
           <?php echo "<a href='verification.php?id=".$row['id']."'>Verify It as Correct</a>";?>
